...
`build.d/`) excludes `kwin`/`kwin-libs`/`kwin-common`/`libplasma`/`kscreenlocker`/
`plasma-workspace*`/`plasma-desktop`/`kdecoration`/`kf6-kwindowsystem` from every
dnf transaction in this build. Root cause it guards against: `dnf install` for an
unrelated package can still resolve a newer `kwin`/`libplasma` against live
Fedora/COPR repo metadata at build time without a matching `kscreenlocker` build
existing yet, producing an undefined-symbol crash in `kwin_wayland` at login
(black screen, no login prompt) — see
`docs/research/kwin-screenlocker-abi-2026-07-26/` and `-2026-08-08/`. Verified by
a `tests/smoke.sh` check that `kwin`/`kscreenlocker` share the same major.minor.
