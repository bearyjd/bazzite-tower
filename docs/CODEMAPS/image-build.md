...
| 275–347| **OpenSnitch** | interception engine for [Snitchwatch](https://github.com/bearyjd/snitchwatch). dnf: `libnetfilter_queue`, `nftables` first — the extraction resolves no deps and `libnetfilter_queue.so.1` is a hard `DT_NEEDED` of `opensnitchd` absent from the base, so without it the daemon can't exec. Not in Fedora/RPM Fusion — `curl` the pinned v1.8.0 upstream release RPM, verify sha256 (no published GPG key to trust instead), extract with `rpm2cpio \| cpio` (not `dnf`/`rpm install` — RPM's %post needs a live systemd bus, and on an RPM-6/SQLite-rpmdb base the transaction's db writes were found to not survive the buildah layer commit), then `install` the staged config from `/usr/share/bazzite-tower/` over the RPM's (must run *after* extraction — cpio's overwrite of a `COPY`ed file is mtime-dependent), enable `opensnitch.service` ourselves. Daemon only, no GUI package (`opensnitch-ui` conflicts with Snitchwatch). Config deltas: `Server.Address` → the bridge's `127.0.0.1:50051`; `ProcMonitorMethod` → `proc` (bundled eBPF module won't load on 6.19/7.x, snitchwatch#6); `DefaultAction: allow` = fail open during rollout while the bridge is still hand-installed |
