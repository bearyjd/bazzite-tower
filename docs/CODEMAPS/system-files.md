...
- `/etc/systemd/system/docker.service.d/libvirt-forwarding.conf` → runs `/usr/local/libexec/docker-libvirt-forwarding` after Docker creates `DOCKER-USER`; it does not alter Docker's FORWARD policy, Docker-managed chains, or nftables tables
- `/etc/libvirt/hooks/network` and `/etc/NetworkManager/dispatcher.d/90-docker-libvirt-forwarding` → asynchronously request reconciliation on libvirt NAT lifecycle or route-change events; both always return success and the dispatcher invokes it only while Docker is active
