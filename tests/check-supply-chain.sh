...
# The attested predicate is a strict installed-RPM inventory from the exact
# local candidate. Do not restore the unbounded whole-filesystem Syft scan,
# archive export, or external scanner download.
assert 'uses: anchore/sbom-action@' not in workflow
assert 'uses: anchore/sbom-action/download-syft@' not in workflow
assert 'podman save --format oci-archive' not in workflow
assert 'syft/releases/download' not in workflow
assert 'podman run --rm --entrypoint /usr/bin/rpm "${candidate}"' in workflow
assert "-qa --qf '%{NAME}\\t%{EPOCHNUM}\\t%{VERSION}\\t%{RELEASE}\\t%{ARCH}\\n' > \"${inventory}\"" in workflow
assert 'python3 ci/rpm-inventory-spdx.py --input "${inventory}" --output "${sbom}"' in workflow
assert 'test -s "${sbom}"' in workflow
assert 'startswith("SPDX-")' in workflow
assert 'wc -l < "${inventory}"' in workflow
assert 'tests/test-rpm-inventory-spdx.sh' in workflow
assert 'image: ${{ env.IMAGE_REGISTRY }}/${{ env.IMAGE_NAME }}@${{ steps.push_candidate.outputs.digest }}' not in workflow
assert workflow.index('- name: Generate SPDX installed-RPM inventory from local candidate') < workflow.index('- name: Push candidate to GHCR')
